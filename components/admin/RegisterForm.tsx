  ...
  return (
    <form action={action} className="space-y-5">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <label className="block md:col-span-2"><span className="block text-[12.5px] font-semibold mb-1.5">{t('slipFile')}</span><input name="slipFile" type="file" accept="image/jpeg,image/png,image/webp,application/pdf" aria-invalid={Boolean(state.ok === false && state.fieldErrors.slipFile)} aria-describedby={state.ok === false && state.fieldErrors.slipFile ? 'slipFile-error' : undefined} className="w-full border border-line rounded-lg px-3 py-2.5 bg-paper focus:outline-2 outline-honey-deep border-honey file:mr-3 file:rounded-md file:border-0 file:bg-honey-soft file:px-3 file:py-1.5 file:text-[13px] file:font-semibold file:text-ink" /><FieldError id="slipFile-error" messages={state.ok === false ? state.fieldErrors.slipFile : undefined} /><p className="text-xs text-ink-soft mt-1">{t('slipFileHelp')}</p></label>
